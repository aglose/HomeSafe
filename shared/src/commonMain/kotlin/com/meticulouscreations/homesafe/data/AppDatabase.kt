...
/**
 * 10 -> 11: the property plan (a bird's-eye Home layout with cameras placed on it) was removed,
 * and its two tables go with it. Nothing else referenced them, so the drop is the whole change;
 * an install that had switched to the map lands back on the list.
 */
@DeleteTable(tableName = "CameraPlacementEntity")
@DeleteTable(tableName = "HomeLayoutEntity")
class PropertyPlanDropped : AutoMigrationSpec
...
@Database(
    ...
    autoMigrations = [
        ...
        // 9 -> 10: the property plan — where each camera sat and which layout the Home tab was
        // showing, both new tables. Kept so an install still on 9 can step through 10 to 11.
